...
function rsiTouchFlipComboKey(prefs) {
  if (!prefs) {
    return "";
  }
  return [prefs.rsiLen, prefs.osLevel, prefs.obLevel, prefs.maxStack].join("|");
}

function compactFitPrefs(prefs) {
  if (!prefs) {
    return null;
  }
  return {
    ...
    cycleSlEnabled: prefs.cycleSlEnabled,
    cycleSlPct: prefs.cycleSlPct
  };
}

/**
 * В таблицу / бота: зелёный Test, иначе чемпион Обзора (тогда «нельзя»).
 * @param {{ best?: object|null, bestTradable?: object|null }|null|undefined} result
 * @returns {{ pick: object|null, overviewBest: object|null }}
 */
export function pickRsiTouchFlipTradeRow(result) {
  const pick = result?.bestTradable || result?.best || null;
  const champ = result?.best || null;
  const overviewBest =
    pick &&
    champ &&
    rsiTouchFlipComboKey(pick.prefs) !== rsiTouchFlipComboKey(champ.prefs)
      ? champ
      : null;
  return { pick, overviewBest };
}

function compactFitRow(row, overviewOverride) {
  if (!row) {
    return null;
  }
  return {
    combo: row.combo,
    prefs: compactFitPrefs(row.prefs),
    overview: overviewOverride || compactRsiTouchFlipOverview(row.overview),
    train: compactRsiTouchFlipOverview(row.train),
    test: compactRsiTouchFlipOverview(row.test),
    verdict: row.verdict
  };
}

/**
 * Поле `best` для воркера скринера: торгуемый набор + справка по чемпиону Обзора.
 * @param {{ best?: object|null, bestTradable?: object|null }|null|undefined} result
 * @param {object|null} [overviewOverride]
 */
export function packRsiTouchFlipScreenerBest(result, overviewOverride = null) {
  const { pick, overviewBest } = pickRsiTouchFlipTradeRow(result);
  const packed = compactFitRow(pick, overviewOverride);
  if (!packed) {
    return null;
  }
  packed.overviewBest = compactFitRow(overviewBest);
  return packed;
}
