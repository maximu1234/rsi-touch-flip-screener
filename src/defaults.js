...
export function normalizeConfig(raw = {}) {
  ...
  const universe =
    String(src.universe || DEFAULT_CONFIG.universe).toLowerCase() === "top100"
      ? "top100"
      : "all";
  const workers = Math.max(1, Math.min(8, Math.round(Number(src.workers) || DEFAULT_CONFIG.workers)));
  const fetchConcurrency = Math.max(
    1,
    Math.min(6, Math.round(Number(src.fetchConcurrency) || DEFAULT_CONFIG.fetchConcurrency))
  );
  const symbols = Array.isArray(src.symbols)
    ? src.symbols.map((s) => String(s || "").replace(/\.P$/i, "").trim().toUpperCase()).filter(Boolean)
    : [];
  return {
    exchange: String(src.exchange || DEFAULT_CONFIG.exchange).toLowerCase() === "bingx" ? "bingx" : "bybit",
    universe,
    ...
    sizeMode: String(src.sizeMode || DEFAULT_CONFIG.sizeMode).toLowerCase() === "equal" ? "equal" : "average",
